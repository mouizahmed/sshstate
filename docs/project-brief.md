...
> Status: stable MVP on macOS and Linux; no v1 target or milestone roadmap.
...
The current product is single-user, self-hosted, CLI-first, with no telemetry. Tunnels, reverse proxies, and VPNs are deployment choices. The client consumes an HTTPS URL and does not implement a tunnel service, identity-based certificate issuer, team system, or generic secrets manager.
...
sshstate session-create <alias> [--key <key>]... [--ttl <duration>]
sshstate sessions
sshstate session-revoke <session-id>
...
Rotation verbs are specified but unscheduled. Session verbs are the final planned feature and follow relay move/rebuild. The withdrawn Git helper adds no command.

**Withdrawn on 2026-09-15: `sshstate tui`.** The CLI is the interface. Everything the TUI was to provide — host browsing, key references and fingerprints, lock and sync status, conflict review and resolution, and eventual rotation progress — must be legible from the CLI.
...
Generate complete values for managed connection fields: HostName, User, Port (22 when omitted in a newly created record), ProxyJump (none when absent), ordered IdentityFile entries, IdentityAgent, IdentitiesOnly, ForwardAgent, and the managed host-trust policy. Resolve an omitted new-host User to the creating machine's username once and store it; do not reinterpret it on every device. Other options inherit local OpenSSH settings. Do not freeze all OpenSSH defaults.
...
The reason to own the agent is control over decrypted-key lifetime and signing availability, keeping private keys out of plaintext files, and leaving the user's global agent selection alone. Automatic prompt-on-sign is not planned and must not lead the README or demo.
...
Explicitly set UpdateHostKeys no. Custom UserKnownHostsFile changes its default anyway; automatic rotation and capture-file compaction are not planned. A host-key change requires explicit trust review. Successful verification against a previously trusted key is distinct from accepting a new TOFU key.
...
Each device has separate signing and recipient-encryption key roles, using independent keypairs with no cross-algorithm conversion: ML-DSA-65 for signatures and an age hybrid (ML-KEM-768 + X25519) identity for recipient encryption. The §4.8 assessment is complete and this suite has been used from genesis; see decision record 0001. Devices receive vault keys in signed recipient-encrypted bundles.
...
Reject duplicate JSON keys, unknown mandatory format versions, malformed encodings, and oversized objects. Local KDF readers bound allocation to 1 GiB, iterations to 10, and lanes to 16; unsupported headers fail clearly rather than allocate without limit. Writers use the specified default until an explicit migration changes it.
...
Both device private keys are encrypted under the local KEK. Sync and reconciliation require unlock. Sync explicitly and once after unlock when configured; there is no periodic background sync. Network failure never blocks local SSH use.

Keep parsed signers in daemon memory only while unlocked. Wipe owned mutable buffers on lock/exit, avoid secret strings and logs where APIs permit, disable core dumps where supported, and use restrictive file permissions. Go cannot guarantee erasure of every library/runtime copy or prevent privileged memory inspection or all swap exposure. memguard and OS keychain integration are not planned. Do not claim a moving Go heap GC or complete zeroization.
...
Use explicit suite/version identifiers bound into genesis, membership, pairing transcripts, envelope context, signatures, and exports. Unknown suites fail closed. No relay-selected downgrade or silent fallback. One suite per vault per epoch; there is no mixed-suite mode. Changing suite is an epoch transition under §7.1, not an in-place substitution. sshstate used the adopted post-quantum suite from genesis and therefore needs no classical-to-hybrid vault migration path.
...
Use SQLite for the single-user relay too, in a persistent container volume. Maintain current record heads plus an immutable accepted-change log. There is no tombstone or accepted-log garbage collection. Master-key rotation publishes a staged, authenticated epoch transition and re-encrypted current snapshot (§7.1); old log entries remain historical and are not overwritten in place.
...
An accepted write atomically updates the head, appends the log with seq, and stores its idempotency outcome. Rejected conflict outcomes are also stable for that mutation ID. Retain mutation outcomes with the accepted log.
...
PUT  /v1/records/:id
POST /v1/rotations
GET  /v1/rotations?after_epoch=:epoch
GET  /v1/rotations/active
GET  /v1/rotations/:id
GET  /v1/rotations/:id/staged
PUT  /v1/rotations/:id/staged/:item
GET  /v1/rotations/:id/manifest
...
Set ForwardAgent no in generated config. The primary sshstate agent does not claim reliable protocol-level detection of every forwarded request; explicit overrides and forwarding of that socket are unsupported. Per-signature confirmation is not planned. Normal lock or session expiry does not end SSH sessions already authenticated.
...
Resumable master-key rotation is specified in `docs/rotation.md`, is not implemented, and is not scheduled. It rotates both metadata and secret keys to fresh random values and increments the epoch; it is separate from changing a local unlock password and from remote SSH host-key rotation.
...
Rotation requests and uploads are idempotent. `rotation-status` distinguishes staging, committed, locally finalizing, and finalized states; `rotation-resume` continues the recorded operation after interruption or a lost response. Before commit, `rotation-abort` can discard staging and release the write freeze; it cannot undo an already committed epoch. A remaining authorized device must be able to abort a stranded pre-commit operation if the initiator is lost. After commit, readers reject regressions and recovery completes forward. The exact manifest, transaction, timeout, discovery, retention, and recovery state machine is specified in `docs/rotation.md`.
...
### 7.3 Unscheduled and closed features

Master-key rotation is unscheduled and demand-triggered under §7.1. Automatic
TTY/desktop prompts, OS keychain unlock, per-signature confirmation, primary-agent
forwarding constraints, periodic sync, automatic remote host-key rotation,
capture-file compaction, any TUI, broad config import, the Git signing helper,
Windows, additional package ecosystems, GUI/mobile, teams, and field-level
merging are not planned.

Metadata-only clients are not planned. The two-key split limits the secrets they
can decrypt, but a maliciously served browser can still steal metadata.
Read-only UI and metadata-only encryption do not make server-delivered
JavaScript trustworthy.

## 8. Implementation history and verification

Milestones 0 through 3a record completed implementation stages. They are
history, not a remaining release roadmap.
...
CI runs formatting checks, build, vet, tests, race checks where supported, and dependency vulnerability checks. Fuzz bounded config, record, and control-message parsers. Validate completed stages and each new change in proportion to its risks; do not substitute mocks for the native SSH demo.
...
The original entries were accepted on 2026-09-12 unless an amendment or later decision is shown. Their mechanisms are specified above; implementation may reveal defects requiring a recorded amendment. R12's assessment is complete, so R1 and R12 state the same adopted suite rather than a baseline pending a decision.
...
| R14: distribution | Signed multi-arch release artifacts and a Homebrew tap for macOS/Linux in M3a; distribution repositories, OS package signing, and Windows packaging are not planned | A tool installed on every machine needs a real install path; broader packaging is permanent maintenance cost without current value |
| R16: relay hosting | **Accepted 2026-09-17**: one hardened relay container, embedded SQLite volume, loopback upstream, external HTTPS proxy, digest pinning; no PostgreSQL or bundled proxy | A single-vault relay needs no separate database service |
