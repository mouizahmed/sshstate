...
Auth: the one-time bootstrap secret, 256 bits, either generated by the relay
when none is configured or supplied in a secret file. Both base64 alphabets are
accepted in the header, padded or not, since the value compared is the decoded
256 bits. The relay stores only the hash of those bytes, compares in constant
time, consumes it atomically when the single account is created, and disables
bootstrap permanently thereafter. Replacing the secret afterwards does not
reopen registration. There is no open registration. A generated secret is
written once to the relay's startup log, and never again; no other request or
response body logs it.
