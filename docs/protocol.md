...
Auth: the one-time bootstrap secret, 256 bits, either issued by the relay on
the operator's request or supplied in a secret file. Both base64 alphabets are
...
reopen registration. There is no open registration and the secret is never
written to logs.
