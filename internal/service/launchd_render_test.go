...
func TestPlistIsWellFormedAndPinsTheLayout(t *testing.T) {
	l := testLayout()
	body, err := service.RenderPlist("/usr/local/bin/sshstate", l)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlistIsByteStableAcrossRenders(t *testing.T) {
	l := testLayout()
	first, err := service.RenderPlist("/bin/sshstate", l)
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 20; i++ {
		again, err := service.RenderPlist("/bin/sshstate", l)
		if err != nil {
			t.Fatal(err)
		}
		if again != first {
...
func TestPlistEscapesPathsThatWouldBreakTheXML(t *testing.T) {
	...
	body, err := service.RenderPlist(`/bin/ssh"state`, l)
	if err != nil {
		t.Fatal(err)
	}
	if err := xml.Unmarshal([]byte(body), new(struct {
